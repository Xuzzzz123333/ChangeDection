...
class Model(nn.Module):
    def __init__(self, opt):
        ...
        self.model = get_model(
            ...
            device=(
                f"cuda:{opt.device_id}"
                if torch.cuda.is_available() and opt.device_id >= 0
                else "cpu"
            ),
            n_layers=opt.n_layers,
            extract_ids=opt.extract_ids,
            use_dynamic_policy=opt.use_dynamic_policy,
            use_head_policy=opt.use_head_policy,
        ...
        if opt.is_main_process:
            ...
            if getattr(opt, "use_dynamic_policy", False):
                ...
                print(f"policy_threshold = {opt.policy_threshold}")
                ...
                print(
                    ...
                    f"{opt.policy_force_keep_during_warmup}"
                )
                print(
    ...
    def _collect_dynamic_policy_state(self):
        network = self._unwrap_model(self.model)
        dino = getattr(getattr(network, "encoder", None), "dino", None)
        if dino is None or not hasattr(dino, "dynamic_policy_debug_state"):
            return None
        return dino.dynamic_policy_debug_state()

    def get_dynamic_policy_metadata(self):
        network = self._unwrap_model(self.model)
        dino = getattr(getattr(network, "encoder", None), "dino", None)
        if dino is None or not hasattr(dino, "dynamic_policy_metadata"):
            return None
        return dino.dynamic_policy_metadata()

    def _sync_dynamic_policy_epoch(self):
        """Propagate the outer training epoch into the DINO adapter so that the
        adapter can decide whether to bypass the learned policy during warmup.
        Safe to call even when dynamic policy is disabled.
        """
        if not getattr(self.opt, "use_dynamic_policy", False):
            return
        network = self._unwrap_model(self.model)
        dino = getattr(getattr(network, "encoder", None), "dino", None)
        if dino is not None:
            dino.current_epoch = int(getattr(self, "current_epoch", 0))
    ...
    def _dynamic_policy_target_ratio(self, epoch):
        if not getattr(self.opt, "use_dynamic_policy", False):
            return 1.0
        warmup_epochs = int(getattr(self.opt, "policy_warmup_epochs", 0))
        anneal_epochs = int(max(0, getattr(self.opt, "policy_anneal_epochs", 0)))
        target_ratio = float(getattr(self.opt, "target_compute_ratio", 1.0))
        if epoch < warmup_epochs:
            return 1.0
        if anneal_epochs <= 0:
            return target_ratio
        anneal_index = epoch - warmup_epochs
        if anneal_index < anneal_epochs:
            progress = float(anneal_index + 1) / float(anneal_epochs)
            return 1.0 + progress * (target_ratio - 1.0)
        return target_ratio

    def _dynamic_policy_budget_lambda(self, epoch):
        if not getattr(self.opt, "use_dynamic_policy", False):
            return 0.0
        if epoch < int(getattr(self.opt, "policy_warmup_epochs", 0)):
            return 0.0
        return float(getattr(self.opt, "policy_budget_weight", 0.0))
    ...
    def forward(self, x1, x2, label):
        ...
        if getattr(self.opt, "use_dynamic_policy", False):
            ...
            self.last_aux_losses["dynamic_policy_mean_block_keep"] = float(
                dynamic_state.get("mean_block_keep", 1.0)
            )
            ...
            self.last_aux_losses["dynamic_policy_max"] = float(
                dynamic_state.get("policy_max", 1.0)
            )
    ...
    def get_dynamic_policy_state(self):
        return self._collect_dynamic_policy_state()
...
def create_model(opt):
    ...
    device = (
        torch.device(f"cuda:{opt.device_id}")
        if torch.cuda.is_available() and opt.device_id >= 0
