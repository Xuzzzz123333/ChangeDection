...
class DINOBlockChangeAwareLocalAdapter(nn.Module):
    def __init__(
        ...
        change_delta_mode: str = "antisymmetric",
        change_mixer_enable: bool = False,
        change_mixer_kernel_size: int = 7,
        change_mixer_residual_scale: float = 1.0,
    ):
        super().__init__()
        if change_delta_mode not in {"antisymmetric", "symmetric"}:
            raise ValueError(
                "change_delta_mode must be one of {'antisymmetric', 'symmetric'}."
            )
        self.block = block
        self.num_prefix_tokens = int(max(0, num_prefix_tokens))
        self.change_delta_mode = change_delta_mode
        self.norm = nn.LayerNorm(dim)
        self.local_branch = DINOConvTokenBranch(
            dim=dim,
            kernel_size=kernel_size,
    ...
    def _forward_pair(self, x1: torch.Tensor, x2: torch.Tensor):
        ...
        delta_update = delta_map * self.gamma_delta.view(1, -1, 1, 1)

        update1 = self.residual_scale * (
            local_gate * local1 * self.gamma_self.view(1, -1, 1, 1)
            + delta_update
        )
        if self.change_delta_mode == "antisymmetric":
...
class DINOV3Wrapper(nn.Module):
    def __init__(
        ...
        local_conv_change_delta_mode="antisymmetric",
        ...
        self.local_conv_change_delta_mode = str(local_conv_change_delta_mode)
        ...
        self.use_dynamic_policy = bool(use_dynamic_policy)
        ...
        self.num_prefix_tokens = infer_num_prefix_tokens(
            self.model,
            override=(
                self.num_prefix_tokens_override
                if self.num_prefix_tokens_override >= 0
                else None
            ),
        )
        ...
        if self.use_dynamic_policy:
            self.inject_dynamic_policy()
            if self.policy_hidden_dim <= 0:
                self.policy_hidden_dim = max(self.model.embed_dim // 4, 256)
    ...
    @staticmethod
    def _unwrap_transformer_block(block: nn.Module) -> nn.Module:
    ...
    def _infer_num_heads(self) -> int:
        ...
        if not blocks:
            raise AttributeError("Cannot infer DINO num_heads because model.blocks is missing.")
        block = self._unwrap_transformer_block(blocks[0])
        attn = getattr(block, "attn", None)
        if attn is None:
            raise AttributeError("Cannot infer DINO num_heads because block.attn is missing.")
        for attr_name in ("num_heads", "heads", "n_heads"):
            if hasattr(attn, attr_name):
                return int(getattr(attn, attr_name))
    ...
    def inject_dynamic_policy(self):
        if not self.use_dynamic_policy:
            return
        if self.block_policy_mode not in {"record_only", "soft_residual", "hard_skip"}:
            raise ValueError(
                ...
                adapter_kwargs.update(
                    {
                        ...
                        "change_delta_mode": self.local_conv_change_delta_mode,
    ...
    def dynamic_policy_metadata(self):
        return {
    ...
    def dynamic_policy_budget_loss(
        self,
        target_ratio: float,
    ...
    def _policy_from_logits(self, logits, kind: str):
        if logits is None:
            return None, None
        raw_prob = torch.sigmoid(logits / max(self.policy_temperature, 1e-6))
        effective = sample_policy(
            logits=logits,
            mode=self.policy_mode,
            temperature=self.policy_temperature,
            hard=self.policy_hard,
    ...
    def _apply_token_policy_to_tokens(self, tokens: torch.Tensor, token_policy):
        if token_policy is None:
            return tokens
        if token_policy.shape[1] != tokens.shape[1] - self.num_prefix_tokens:
            raise ValueError(
                "Token policy length does not match patch-token count. "
                f"policy={tuple(token_policy.shape)}, tokens={tuple(tokens.shape)}, "
                f"num_prefix_tokens={self.num_prefix_tokens}"
            )
        prefix_tokens = tokens[:, : self.num_prefix_tokens]
        patch_tokens = tokens[:, self.num_prefix_tokens :] * token_policy[:, :, None].to(
            dtype=tokens.dtype
        )
        return torch.cat([prefix_tokens, patch_tokens], dim=1)
    ...
    def _apply_block_policy_to_outputs(self, x_prev_list, x_out_list, block_policy):
        if block_policy is None or self.block_policy_mode == "record_only":
            return x_out_list
        gated_outputs = []
        for x_prev, x_out in zip(x_prev_list, x_out_list):
            gated_outputs.append(
                x_prev
                + block_policy[:, None, :].to(dtype=x_out.dtype) * (x_out - x_prev)
            )
    ...
    def _collect_dynamic_policy_state(
        ...
        if reference is None:
            ...
            policy_std = torch.zeros((), device=device)
        else:
            policy_min = reference.min()
    ...
    def forward_pair(self, x1: torch.Tensor, x2: torch.Tensor):
        ...
        self.last_cgla_priors = []
        self.last_dynamic_policy_state = None
        use_grad = self.training and (
            self.use_lora or self.local_conv_enable or self.use_dynamic_policy
        )
