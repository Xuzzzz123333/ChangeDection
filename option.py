...
class Options:
    ...
    def init(self):
        ...
        self.parser.add_argument(
            "--dino_local_conv_change_delta_mode",
            type=str,
            default="antisymmetric",
            choices=["antisymmetric", "symmetric"],
            help="delta injection rule used by the change-aware DINO local branch; antisymmetric uses +delta/-delta while symmetric uses +delta/+delta for ablations",
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            "--use_dynamic_policy",
            action="store_true",
            help="enable AdaViT-style dynamic policy learning on top of the DINO encoder",
        )
        self.parser.add_argument(
            "--use_head_policy",
            action="store_true",
            help="learn per-sample per-layer attention-head keep policies",
        )
        self.parser.add_argument(
            "--use_block_policy",
            action="store_true",
            help="learn per-sample per-layer transformer block keep policies",
        )
        self.parser.add_argument(
            "--block_policy_mode",
            type=str,
            default="record_only",
            choices=["record_only", "soft_residual", "hard_skip"],
            help="block policy behavior; first version records or gates residual updates but does not physically skip compute",
        )
        self.parser.add_argument(
            "--use_token_policy",
            action="store_true",
        ...
        self.parser.add_argument(
            ...
            default="soft",
            choices=["soft", "gumbel"],
            help="policy sampling mode used for dynamic head/block/token gates",
        )
        self.parser.add_argument(
            "--policy_hard",
            action="store_true",
            help="use straight-through hard gates for dynamic policy sampling",
        )
        self.parser.add_argument(
            "--policy_temperature",
            type=float,
            default=1.0,
            help="temperature used by dynamic policy sigmoid or gumbel-sigmoid sampling",
        )
        self.parser.add_argument(
            "--policy_threshold",
            type=float,
            default=0.5,
            help="threshold used by hard dynamic policy gates at evaluation or with straight-through sampling",
        )
        self.parser.add_argument(
            "--head_topk_ratio",
            type=float,
        ...
        self.parser.add_argument(
            ...
            type=float,
            default=0.90,
            help="target average keep ratio used by the dynamic policy budget regularizer",
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            ...
            type=int,
            default=3,
            help="number of warmup epochs before enabling dynamic policy budget pressure",
        )
        self.parser.add_argument(
            "--policy_anneal_epochs",
            type=int,
            default=5,
            help="number of epochs used to anneal the target compute ratio from 1.0 to the target value",
        )
        self.parser.add_argument(
            "--policy_force_keep_during_warmup",
            action=argparse.BooleanOptionalAction,
            default=True,
            help="during policy_warmup_epochs, force all dynamic policy gates to 1.0 "
                 "and skip calling the policy net so its parameters receive no "
                 "segmentation-loss gradient. Use --no-policy_force_keep_during_warmup "
                 "to reproduce the legacy (pre-fix) behavior where the learned gate "
                 "already shapes attention during warmup.",
        )
        self.parser.add_argument(
            "--policy_hidden_dim",
            type=int,
            default=256,
            help="hidden dimension used by the lightweight AdaViT-style relation policy MLP; <=0 falls back to embed_dim//4 or 256",
        )
        self.parser.add_argument(
            "--num_prefix_tokens",
            type=int,
        ...
        self.parser.add_argument(
            ...
            type=float,
            default=1.0,
            help="head-policy contribution to the dynamic policy compute-cost estimate",
        )
        self.parser.add_argument(
            "--policy_block_weight",
            type=float,
            default=0.0,
            help="block-policy contribution to the dynamic policy compute-cost estimate",
        )
        self.parser.add_argument(
            "--policy_token_weight",
            type=float,
    ...
    def parse(self):
        ...
        if self.opt.use_dynamic_policy and not (
        ...
        if self.opt.policy_anneal_epochs < 0:
            raise ValueError("--policy_anneal_epochs must be >= 0.")
        if self.opt.policy_head_weight < 0:
            raise ValueError("--policy_head_weight must be >= 0.")
        if self.opt.policy_block_weight < 0:
